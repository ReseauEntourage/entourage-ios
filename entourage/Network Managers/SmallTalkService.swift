...
struct SmallTalkService: ParsingDataCodable {
    ...
    static func prepareUploadWith(smallTalkId: String, image: UIImage, message: String?, completion: @escaping (_ result: Bool) -> Void) {
        guard let token = UserDefaults.token else {
            completion(false)
            return
        }

        let endpoint = String(format: API_URL_SMALLTALK_PREPARE_IMAGE_POST_UPLOAD, smallTalkId, token)
        ...
        AuthService.prepareUploadPhotoS3(endpoint: endpoint) { json, error in
            ...
            self.uploadToS3(urlS3: presignedUrl, image: image) { isOk in
                if isOk {
                    self.postWithImageAndText(imageKey: uploadKey, smallTalkId: smallTalkId, message: message, completion: completion)
                } else {
                    completion(false)
                }
            }
    ...
    private static func postWithImageAndText(imageKey: String, smallTalkId: String, message: String?, completion: @escaping (_ result: Bool) -> Void) {
        guard let token = UserDefaults.token else {
            completion(false)
            return
        }

        let endpoint = "smalltalks/\(smallTalkId)/chat_messages?token=\(token)"
