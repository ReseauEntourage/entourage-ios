...
let kAPIGetSuggestFilteredNeighborhoods = "neighborhoods?token=%@&page=%d&per=%d&travel_distance=%.2f&latitude=%.6f&longitude=%.6f&interests[]=%@"
...
let kAPIEventGetAllFilteredDiscover = "outings?token=%@&page=%d&per=%d&period=future&travel_distance=%.2f&latitude=%.6f&longitude=%.6f&interests[]=%@"
