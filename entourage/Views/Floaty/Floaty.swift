...
//  Mod for adding blur background By Jr 2022.
//
...
open class Floaty: UIView {
    ...
    /**
     For adding blurry overlay
     */
    @objc open var addBlurOverlay: Bool = false
    @objc open var blurOverlayStyle: UIBlurEffect.Style = .regular
    @objc open var blurEffectView:UIVisualEffectView? = nil
    ...
    fileprivate func addBlurryBG() {
        let blurEffect = UIBlurEffect(style: blurOverlayStyle)
        ...
        if(items.count > 0){
            ...
            if autoCloseOnOverlayTap {
                if addBlurOverlay {
                    ...
                    let _touch = UITapGestureRecognizer(target: self, action:  #selector(close))
                    blurEffectView?.addGestureRecognizer(_touch)
                }
                else {
