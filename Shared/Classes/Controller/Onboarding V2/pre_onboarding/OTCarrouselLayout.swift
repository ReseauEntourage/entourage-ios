...
class OTCarrouselLayout: UICollectionViewLayout {
    
    // MARK: - Public Properties
    init(cellWidth:CGFloat,cellHeight:CGFloat) {
        super.init()
        itemSize = CGSize(width: cellWidth, height: cellHeight)
        print("item Size after init : \(itemSize)")
    }
  ...
  //  private let itemSize = CGSize(width: 228, height: 267)
    private lazy var itemSize = CGSize(width: 0, height: 0)
    ...
    override func targetContentOffset(forProposedContentOffset proposedContentOffset: CGPoint, withScrollingVelocity velocity: CGPoint) -> CGPoint {
        ...
        print("TargetContentOffset : \(CGPoint(x: closestAttribute.center.x - collectionViewMidX, y: proposedContentOffset.y))")
